...
class GroupQAHandler(FutureHandler):

	def initialize(self, conf, schedules, cache, session_manager):
		super(GroupQAHandler, self).initialize(conf, schedules, cache, session_manager)
	...
	def post(self):
		...
		detlog.info("[groupqa] [BEGIN] [REQUEST] [%s]" % (self.request.uri))
		# Info from QQ group
		...
		if is_emoji_query:
			s = 0
			for each in re.finditer("\[(.+?)\]", emoji_reply):
				if s < each.start():
					_msg["content"].append({"type":0, "data": emoji_reply[s:each.start()].encode("utf-8")})
				_msg["content"].append({"type":4, "data": each.group(1).encode("utf-8")})
				s = each.end()
			if s != len(emoji_reply):
				_msg["content"].append({"type":0, "data": emoji_reply[s:].encode("utf-8")})
			...
			if has_at and self.query == "":
				...
				ans_utf8 = result["answer"].encode("utf-8")
				from_utf8 = result["debug_info"]["from"].encode("utf-8")
				# currently only one data element in msg content
				_msg["content"].append({"type":0, "data":ans_utf8})
...
def make_app(global_conf, schedules, cache, session_manager):
	...
	app = tornado.web.Application(
		[
			(r'/', GroupQAHandler, dict(conf=global_conf, schedules=schedules, cache=cache, session_manager=session_manager)),
